#!/usr/bin/env bash
#==============================================================================
# SLURM Dorado MOD Basecalling + Demultiplexing Script
# Project  : NGS-26-EQ
# Sample   : various
# Platform : PromethION 2i (P2i)
# Model    : SUP v5.2.0 + modbase 6mA / 4mC_5mC (expliciet gepind)
# Dorado   : v2.1.1 (release 19-02-2026, ONT)
# Env      : basecall-qc (mamba; samtools/cramino/modkit/nanoplot voor QC)
...
#-----------------------------Output / error logs------------------------------
#SBATCH --output=/lustre/scratch/WUR/WBVR/harde004/NGS-26-EQ/NGS-26-EQ-1/20260730_1322_P2I-00116-A_PBM40282_b639ac1a/dorado-mod-output_%j.txt
#SBATCH --error=/lustre/scratch/WUR/WBVR/harde004/NGS-26-EQ/NGS-26-EQ-1/20260730_1322_P2I-00116-A_PBM40282_b639ac1a/dorado-mod-error_%j.txt

#-----------------------------Job-informatie-----------------------------------
#SBATCH --job-name=VetB-mod-GPU
...
SCRIPT_NAME="01a_dorado_basecall_mod.sh"
...
OUT_BAM="${OUT_DIR}/calls.sup.mod.bam"
...
## Basecalling-parameters — modellen EXPLICIET gepind.
## NB: er bestaat geen sup@v6.0 — v6.0 is HAC-only.
MODEL_SPEC="sup@v5.2.0,6mA@v1,4mC_5mC@v1"
KIT=SQK-NBD114-96

## Provenance-bestand (machine-leesbaar, voor M&M / supplementary)
mkdir -p "${OUT_DIR}"
PROVENANCE="${OUT_DIR}/provenance_mod_${SLURM_JOB_ID:-manual}_$(date +%Y%m%d).txt"
...
log "=== Dorado MOD basecalling gestart ==="
...
log "modkit          : $(modkit --version 2>&1 | head -1 || echo 'onbekend')"
...
#==============================================================================
# Dorado MOD basecalling + barcode-classificatie
#==============================================================================
...
log "--- BAM @RG header (basecall + modbase modellen) ---"
samtools view -H "${OUT_BAM}" \
    | grep -m1 '^@RG' \
    | tr '\t' '\n' \
    | grep -E 'basecall_model|modbase_models|runid|PU|PM|DT' \
    | while IFS= read -r line; do log "  ${line}"; done

log "--- Tag-check (eerste read: moves + modbase aanwezig?) ---"
FIRST_TAGS=$( (samtools view "${OUT_BAM}" || true) | head -n1 | tr '\t' '\n' \
    | grep -oE '^(mv|ts|MM|ML|BC):' | sort -u | paste -sd, )
log "  Tags gevonden : ${FIRST_TAGS:-GEEN}"
for tag in mv MM ML; do
    if [[ ",${FIRST_TAGS}," != *",${tag}:,"* ]]; then
        log "  WAARSCHUWING: ${tag}-tag ontbreekt in eerste read!"
    fi
done

log "--- cramino (yield / N50 / Q-score / mod-tags) ---"
cramino "${OUT_BAM}" 2>&1 | while IFS= read -r line; do log "  ${line}"; done

log "--- modkit summary (sanity check MM/ML mod-calls) ---"
modkit summary --threads "${SLURM_CPUS_PER_TASK:-8}" "${OUT_BAM}" 2>&1 \
    | while IFS= read -r line; do log "  ${line}"; done
